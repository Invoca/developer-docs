...
Customer Data
'''''''''''''
Advertiser campaigns may have Customer Data fields applied to them, which will be applied to calls originating through the advertiser campaign.
To apply customer data values to an advertiser campaign, the top level parameter ``customer_data`` should be assigned a hash with each pair's key corresponding to a partner name.
...
''''''''''''''''''''''''
The Customer Data Dictionary is a list of all of the fields that can be applied to the advertiser campaign.
The dictionary is configured by an Invoca representative, and will be returned in each response when viewing or updating an advertiser campaign under the field name ``customer_data_dictionary``.
